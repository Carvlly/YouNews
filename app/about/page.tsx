    ...
    <div className="flex flex-col min-h-screen bg-bg-base">
        ...
        <div className="flex items-center gap-[8px] sm:gap-[12px]">
          ...
          <div className="flex items-center gap-[8px] sm:gap-[12px] text-text-primary min-w-0">
            ...
            <div className="font-mono text-[16px] sm:text-[20px] font-bold text-text-primary truncate">
              YouNews <span className="hidden sm:inline font-normal opacity-50 text-[14px]">/ About</span>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto w-full max-w-2xl mx-auto px-6 py-12">
        <div className="space-y-8">
          <div>
            <h1 className="text-2xl font-bold font-mono text-text-primary mb-2">About YouNews</h1>
            <p className="text-text-secondary text-sm">
              v1.0.4 - High Density News Reader
            </p>
          </div>

          <div className="p-6 bg-bg-card border border-border-base rounded flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <h2 className="text-accent font-bold uppercase tracking-wider text-xs font-mono">Important Notice 01</h2>
              <p className="text-text-primary text-sm leading-relaxed">
                当サイトは <strong>RSSフィードのみ</strong> を対象としてニュースを収集・表示しています。
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <h2 className="text-accent font-bold uppercase tracking-wider text-xs font-mono">Important Notice 02</h2>
              <p className="text-text-primary text-sm leading-relaxed">
                本サービスは現在 <strong>開発中 (Under Development)</strong> です。予期せぬ不具合や仕様変更が発生する可能性があります。
              </p>
            </div>
          </div>
          
          <div className="text-center mt-12">
            <Link href="/" className="inline-block px-6 py-2 bg-bg-hover text-text-primary text-sm font-mono uppercase tracking-wider rounded hover:bg-border-base transition-colors">
              Return to Home
            </Link>
          </div>
        </div>
